...
import { X, Bell, AtSign, Info, MessageCircle, Flag, Shield, Phone, Image as ImageIcon, Mic, FileText, ChevronLeft, Play, Download, Video as VideoIcon, Copy, Check } from 'lucide-react'
...
export default function ProfileModal({ userId, onClose, onMessage }: ProfileModalProps) {
  ...
  const [profileViewer, setProfileViewer] = useState<MediaItem[] | null>(null)
  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-50 flex items-end sm:items-center justify-center sm:p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="relative bg-gradient-to-b from-tg-active to-tg-panel pb-4">
          ...
          <div className="flex flex-col items-center pt-8 px-6">
            <div className="relative">
              <button
                onClick={() => profile.avatar_url && setProfileViewer([{ url: profile.avatar_url, type: 'image', name: 'profile.jpg' }])}
                className={profile.avatar_url ? 'rounded-full transition-transform hover:scale-105 active:scale-95' : ''}
              >
        ...
        {activeTab === 'info' && onMessage && !showReport && (
          <div className="flex justify-center px-6 py-4">
            <button onClick={onMessage} className="flex items-center gap-2 px-8 py-3 rounded-full bg-gradient-to-l from-tg-accent to-tg-accent2 text-white font-medium text-sm shadow-lg shadow-tg-accent/30 hover:shadow-xl hover:shadow-tg-accent/40 hover:scale-[1.03] active:scale-95 transition-all">
        ...
        {showReport ? (
          ...
          <>
            ...
            <div className="px-3 pb-3 space-y-1">
              ...
              <div
                className="flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-tg-hover transition-colors select-none"
                onTouchStart={() => profile.username && startPress(profile.username)}
                onTouchEnd={cancelPress}
                onTouchMove={cancelPress}
                onMouseDown={() => profile.username && startPress(profile.username)}
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-tg-subtext text-xs mt-0.5">{usernameCopied ? 'کپی شد' : 'نام کاربری'}</p>
                </div>
              </div>
                ...
                <div className="grid grid-cols-3 gap-0.5 p-0.5">
                  {photos.map(msg => (
                    <button
                      key={msg.id}
                      onClick={() => {
                        if (selectedPhoto === msg.id) { setSelectedPhoto(null); return }
                        setProfileViewer(photos.map(m => ({url:m.file_url!,type:'image' as const,name:m.file_name||'image.jpg',messageId:m.id,fileUrl:m.file_url||undefined})))
                      }}
                      onPointerDown={() => {
                        const t = setTimeout(() => setSelectedPhoto(msg.id), 550)
                        ;(window as any).__sirachatPhotoPress = t
                      }}
                      onPointerUp={() => { const t=(window as any).__sirachatPhotoPress; if(t) clearTimeout(t) }}
                      onPointerLeave={() => { const t=(window as any).__sirachatPhotoPress; if(t) clearTimeout(t) }}
                    ...
                    >
                      ...
                      <div className={`absolute inset-0 transition-colors ${selectedPhoto===msg.id?'bg-black/35':'bg-black/0 group-hover:bg-black/20'}`} />
                      {selectedPhoto===msg.id && msg.sender_id===currentUserId && (
                        <span
                          onClick={async e => {
                            e.stopPropagation()
                            await supabase.from('messages').update({deleted_at:new Date().toISOString()}).eq('id',msg.id).eq('sender_id',currentUserId)
                            setMediaMessages(prev=>prev.filter(m=>m.id!==msg.id)); setSelectedPhoto(null)
                          }}
                          className="absolute top-2 left-2 w-8 h-8 rounded-full bg-tg-red text-white flex items-center justify-center text-xs"
                        >حذف</span>
                      )}
                    </button>
                  ))}
      ...
      {/* Profile media viewer never closes the profile itself. */}
      {lightbox && (
        <div className="fixed inset-0 bg-black/90 z-[60] flex items-center justify-center" onClick={e => { e.stopPropagation(); setLightbox(null) }}>
          <button onClick={e=>{e.stopPropagation();setLightbox(null)}} className="absolute top-4 left-4 z-10 w-10 h-10 rounded-full bg-black/50 flex items-center justify-center text-white"><X size={24}/></button>
          <img src={lightbox} alt="" className="max-w-full max-h-full object-contain" onClick={e=>e.stopPropagation()}/>
        </div>
      )}
      {profileViewer && (
        <MediaViewer items={profileViewer} startIndex={0} onClose={() => setProfileViewer(null)} />
      )}
      {videoViewer && (
