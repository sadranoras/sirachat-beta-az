      ...
      pc.oniceconnectionstatechange = () => {
        ...
        if (pc.iceConnectionState === 'failed') {
          ...
          iceRestartTimer = setTimeout(async () => {
            ...
            if (iceRestartAttempts >= 3) return
            iceRestartAttempts++
            console.log(`ICE failed — attempting restart #${iceRestartAttempts}`)
            try {
              pc.restartIce()
              if (isCaller) {
      ...
      pc.onconnectionstatechange = () => {
        ...
        if (pc.connectionState === 'failed') {
          ...
          iceRestartTimer = setTimeout(async () => {
            ...
            if (iceRestartAttempts >= 3) return
            iceRestartAttempts++
            console.log(`Connection failed — attempting restart #${iceRestartAttempts}`)
            try {
              pc.restartIce()
              if (isCaller) {
                const restartOffer = await pc.createOffer({ iceRestart: true })
            ...
            } catch (e) { console.error('Connection restart error:', e) }
          }, 1000)
        }
      }
    ...
    <div className="fixed inset-0 bg-tg-bg z-[90] flex flex-col">
      ...
      {audioOutputs.length > 1 && (
        <div className="absolute top-4 right-4 z-30">
          ...
          {outputMenuOpen && (
            <div className="absolute top-11 right-0 bg-tg-panel rounded-xl shadow-2xl border border-tg-hover py-1 min-w-[180px] max-w-[220px]">
              {audioOutputs.map((d) => {
                const isActive = selectedOutput === d.deviceId
                const label = d.label || 'دستگاه صوتی'
                const isEarpiece = /earpiece|speakerphone|default|communications/i.test(label) && !/speaker|headphone|headset/i.test(label)
                return (
                  <button
                    key={d.deviceId}
                    onClick={() => { changeAudioOutput(d.deviceId); setOutputMenuOpen(false) }}
                    className={`w-full flex items-center gap-2 px-3 py-2.5 text-xs transition-colors ${isActive ? 'text-tg-accent bg-tg-accent/10' : 'text-tg-text hover:bg-tg-hover'}`}
                  >
                    {isEarpiece ? <Phone size={14} className="flex-shrink-0" /> : <Volume2 size={14} className="flex-shrink-0" />}
