...
import { Send, Phone, Video, Paperclip, Smile, ArrowRight, Trash2, Reply, MoreVertical, Check, CheckCheck, X, Image as ImageIcon, FileText, Mic, Play, Pencil, Download, Loader2, Megaphone, Plus, FileVideo, PlayCircle, Users, Bookmark, Forward, Copy, Eye, Pin, XCircle, Clock, CheckCircle2, FolderOpen, LockKeyhole, Unlock } from 'lucide-react'
...
async function uploadWithProgress(bucket: string, path: string, body: Blob, onProgress: (pct: number) => void): Promise<string> {
  ...
  // Fast path for small files. Large files use resumable TUS.
  const CHUNK_SIZE = 16 * 1024 * 1024
  const MAX_RETRIES = 5

  const directUpload = () => new Promise<string>((resolve, reject) => {
  ...
  if (body.size <= CHUNK_SIZE) return directUpload()
  ...
  const create = await fetch(`${supabaseUrl}/storage/v1/upload/resumable`, {
    ...
    headers: {
      ...
      'Upload-Metadata': [
        ...
        `contentType ${btoa(body.type || 'application/octet-stream')}`,
  ...
  const sessionUrl = create?.ok
    ? (create.headers.get('Location') || create.headers.get('location'))
    : null

  if (!sessionUrl) return directUpload()

  let offset = 0

  while (offset < body.size) {
    const end = Math.min(offset + CHUNK_SIZE, body.size)
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      try {
        ...
        if (res.ok) {
          ...
          onProgress(Math.round(offset * 100 / body.size))
          sent = true
          break
        }

        lastError = new Error(`Upload chunk failed: ${res.status}`)

        if (res.status === 409 || res.status === 412) {
          const head = await fetch(sessionUrl, {
            method: 'HEAD',
            headers: {
              Authorization: `Bearer ${accessToken}`,
              'Tus-Resumable': '1.0.0',
          ...
          if (head.ok && Number.isFinite(serverOffset) && serverOffset >= 0 && serverOffset <= body.size) {
            offset = serverOffset
            onProgress(Math.round(offset * 100 / body.size))
            sent = true
            break
      ...
      await new Promise(r => setTimeout(r, Math.min(6000, 250 * 2 ** attempt)))
    }
...
import { lockMessage, unlockMessage, isLockedMessage } from '../lib/messageEncryption'
import { sendPushNotification } from '../lib/push'

const VIDEO_EXTENSIONS = ['mp4', 'webm', 'mov', 'avi', 'mkv', 'm4v', 'ogv', '3gp']
function isAudioFile(name: string | null): boolean { return !!name && /\.(mp3|wav|m4a|ogg|aac|flac|opus)$/i.test(name) }
...
export default function ChatView({ chatId, onBack, onCall, onShowProfile, onOpenChatWithUsername, onOpenChatWithPhone, previewMode, previewPrivate, onJoined }: ChatViewProps) {
  ...
  const [lockGroupMessage, setLockGroupMessage] = useState(false)
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`messages-${chatId}`)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages', filter: `chat_id=eq.${chatId}` }, async (payload: any) => {
        ...
        newMsg.reactions = []
        if (newMsg.sender_id !== user.id && document.hidden) { sendPushNotification(chatId, newMsg.id, newMsg.sender_id, newMsg.content || newMsg.file_name || 'پیام جدید', newMsg.message_type).catch(() => {}) }
        setMessages(prev => {
  ...
  const sendMessage = async () => {
    ...
    if (lockGroupMessage && chat?.type === 'group') {
      const password = window.prompt('برای این پیام یک رمز وارد کنید (حداقل ۴ کاراکتر):') || ''
      if (password.length < 4) return
      try {
        content = await lockMessage(content, password)
      } catch (e) {
        window.alert(e instanceof Error ? e.message : 'رمزگذاری پیام انجام نشد')
        return
      }
    }
  ...
  useEffect(() => { const h = (e: Event) => setChatBackground((e as CustomEvent<string>).detail || ''); window.addEventListener('chat-background-change', h); return () => window.removeEventListener('chat-background-change', h) }, [])

  return (
    <div className="flex-1 flex flex-col bg-tg-bg h-full w-full relative overflow-hidden" style={chatBackground ? { backgroundImage: chatBackground, backgroundSize: 'cover', backgroundAttachment: 'fixed' } : undefined}>
      {/* Header */}
      {selectedMsgs.size > 0 ? (
              ...
              <div data-msg-id={msg.id} className={`max-w-[70%] rounded-2xl px-3 py-2 ${showAvatar && selectedMsgs.size === 0 ? 'mr-2' : ''} transition-all duration-200 ${highlightedMsgId === msg.id ? 'bg-tg-accent/30 ring-2 ring-tg-accent' : selectedMsgs.has(msg.id) ? (isMine ? 'bg-tg-active ring-2 ring-tg-accent' : 'bg-tg-panel ring-2 ring-tg-accent') : isMine ? 'bg-tg-active' : 'bg-tg-panel'}`} onDoubleClick={(e) => {
                ...
                {msg.message_type === 'file' && msg.file_url && isVideoFile(msg.file_name) && (
                  <button onClick={() => {
                    ...
                    <video src={cachedUrls[msg.id] || msg.file_url} className="rounded-xl max-w-full max-h-80 object-cover" preload="metadata" />
                ...
                {msg.message_type === 'file' && msg.file_url && isAudioFile(msg.file_name) && (
                  <div className={`rounded-2xl p-3 w-full max-w-[320px] ${isMine ? 'bg-tg-active/80' : 'bg-tg-hover/70'}`}>
                    <div className="flex items-center gap-3 mb-2"><div className="w-11 h-11 rounded-full bg-tg-accent flex items-center justify-center"><Play size={20} className="text-white ml-0.5"/></div><div className="min-w-0 flex-1"><p className="text-sm text-tg-text truncate">{msg.file_name}</p><p className="text-xs text-tg-subtext">آهنگ</p></div></div>
                    <audio controls preload="metadata" src={cachedUrls[msg.id] || msg.file_url} className="w-full h-9" />
                  </div>
                )}
                {msg.message_type === 'file' && msg.file_url && !isVideoFile(msg.file_name) && !isAudioFile(msg.file_name) && (() => {
                  const fileStyle = getFileTypeStyle(msg.file_name || 'file')
                  const progress = downloadProgress[msg.id]
                  const isDownloading = progress !== undefined
                  return (
                  <button onClick={() => !isDownloading && handleDownload(msg)} disabled={isDownloading} className="flex items-center gap-3 bg-black/15 hover:bg-black/25 rounded-xl p-2.5 text-right transition-colors w-full max-w-[280px]">
                ...
                {msg.message_type === 'file' && !msg.file_url && uploadProgress[msg.id] !== undefined && (() => {
                  ...
                  <div className="flex items-center gap-3 bg-black/15 rounded-xl p-2.5 w-full max-w-[280px]">
                    ...
                    <div className="flex-1 min-w-0 overflow-hidden text-left">
                      <p className="text-sm text-tg-text truncate">{msg.file_name}</p>
                      <span className="text-xs text-tg-subtext">در حال ارسال... {progress}%</span>
                    </div>
                    <div className="shrink-0 w-9 h-9 rounded-full flex items-center justify-center bg-tg-hover/60">
                ...
                {msg.content && msg.message_type === 'text' && (
                  isLockedMessage(msg.content) ? (
                    <button
                      type="button"
                      className="flex items-center gap-2 text-tg-text text-sm bg-black/10 hover:bg-black/20 rounded-xl px-3 py-2 text-right w-full"
                      onClick={async () => {
                        const password = window.prompt('رمز این پیام را وارد کنید:') || ''
                        if (!password) return
                        try {
                          const plain = await unlockMessage(msg.content, password)
                          window.alert(plain)
                        } catch {
                    ...
                    >
                      <LockKeyhole size={17} className="text-tg-accent shrink-0" />
                      <span>پیام رمزگذاری‌شده — برای نمایش رمز را وارد کنید</span>
                    </button>
                  ) : (
      ...
      <div className="p-3 bg-tg-panel border-t border-tg-border">
        {previewMode && !isMember ? (
          ...
          <div className="flex items-end gap-2 relative">
            ...
            {chat?.type === 'group' && !editingMsg && (
              <button
                type="button"
                title={lockGroupMessage ? 'پیام رمزدار فعال است' : 'ارسال پیام رمزدار'}
                onClick={() => setLockGroupMessage(v => !v)}
                className={`w-9 h-9 rounded-full hover:bg-tg-hover flex items-center justify-center ${lockGroupMessage ? 'text-tg-accent bg-tg-hover' : 'text-tg-subtext'}`}
              >
                {lockGroupMessage ? <LockKeyhole size={18} /> : <Unlock size={18} />}
              </button>
            )}
            <textarea
              ref={textareaRef}
