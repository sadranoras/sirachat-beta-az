...
import { X, Users, Plus, Trash2, Crown, UserPlus, Info, Image as ImageIcon, Mic, FileText, Play, Download, Megaphone, Camera, Link2, Copy, Check, Eye, EyeOff, AtSign, Search, Loader2, ShieldCheck, ShieldOff, Video as VideoIcon, Forward, Clock, UserCheck, XCircle } from 'lucide-react'
import Avatar from './Avatar'
import { isVideoFile } from '../lib/videoUtils'
import MediaViewer, { MediaItem } from './MediaViewer'

interface GroupInfoModalProps { chat: Chat; onClose: () => void; onChatUpdated?: () => void; onShowProfile?: (userId: string) => void }
...
export default function GroupInfoModal({ chat, onClose, onChatUpdated, onShowProfile }: GroupInfoModalProps) {
  const { user } = useAuth()
  ...
  const [forwardEnabled, setForwardEnabled] = useState(chat.forward_enabled !== false)
  ...
  const toggleForward = async () => {
    ...
    const newVal = !forwardEnabled
    await supabase.from('chats').update({ forward_enabled: newVal }).eq('id', chat.id)
    setForwardEnabled(newVal)
    chat.forward_enabled = newVal
  ...
  const loadJoinRequests = useCallback(async () => {
    if (!isOwner || !chat.is_private) return
    const { data } = await supabase.from('join_requests').select('*').eq('chat_id', chat.id).eq('status', 'pending').order('created_at', { ascending: false })
    const list = (data as any[]) || []
    setJoinRequests(list)
    if (list.length > 0) {
      const { data: profs } = await supabase.from('profiles').select('*').in('id', list.map(r => r.user_id))
      const map: Record<string, Profile> = {}
  ...
  }, [chat.id, isOwner, chat.is_private])

  useEffect(() => { loadJoinRequests() }, [loadJoinRequests])

  useEffect(() => {
    if (!isOwner || !chat.is_private) return
    const channel = supabase.channel(`join-req-${chat.id}`)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'join_requests', filter: `chat_id=eq.${chat.id}` }, () => loadJoinRequests())
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'join_requests', filter: `chat_id=eq.${chat.id}` }, () => loadJoinRequests())
      .subscribe()
    return () => { supabase.removeChannel(channel) }
  }, [chat.id, isOwner, chat.is_private, loadJoinRequests])

  const approveRequest = async (reqId: string, userId: string) => {
    await supabase.from('join_requests').update({ status: 'approved', resolved_at: new Date().toISOString() }).eq('id', reqId)
    await supabase.from('chat_members').insert({ chat_id: chat.id, user_id: userId, role: 'member' })
    loadJoinRequests(); loadMembers()
  }
  const rejectRequest = async (reqId: string) => {
    await supabase.from('join_requests').update({ status: 'rejected', resolved_at: new Date().toISOString() }).eq('id', reqId)
    loadJoinRequests()
  }
      ...
      <div className="bg-tg-panel rounded-t-3xl sm:rounded-3xl w-full sm:max-w-md max-h-[92vh] overflow-y-auto animate-slideUp shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        {activeTab === 'info' ? (
          <>
            ...
            {isOwner && (
              <div className="px-4 py-3 border-b border-tg-hover">
                ...
                <div className="flex items-center justify-between mb-3">
                  <div>
                    <p className="text-tg-text text-sm flex items-center gap-1.5">
                      <Forward size={16} className="text-tg-subtext" />
                      فروارد پیام‌ها
                    </p>
                    <p className="text-tg-subtext text-[11px] mt-0.5">{forwardEnabled ? 'فروارد پیام‌ها فعال است' : 'فروارد پیام‌ها غیرفعال است'}</p>
                  </div>
                  <button
                    onClick={toggleForward}
                    disabled={savingForward}
                    className={`relative w-12 h-7 rounded-full transition-colors flex-shrink-0 ${forwardEnabled ? 'bg-tg-accent' : 'bg-tg-hover'}`}
                  >
                    <div className={`absolute top-0.5 w-6 h-6 rounded-full bg-white transition-transform ${forwardEnabled ? 'left-0.5' : 'left-[22px]'}`} />
                  </button>
                </div>
            ...
            {isOwner && chat.is_private && joinRequests.length > 0 && (
              <div className="p-3 border-t border-tg-hover">
                <h4 className="text-tg-text text-sm font-bold mb-2 flex items-center gap-2"><Clock size={16} className="text-tg-accent" /> درخواست‌های عضویت ({joinRequests.length})</h4>
                <div className="space-y-2">
                  {joinRequests.map(req => {
                    const p = requestProfiles[req.user_id]
                    return (
                      <div key={req.id} className="flex items-center gap-2 p-2 rounded-xl bg-tg-hover/40">
                        ...
                        <button onClick={() => approveRequest(req.id, req.user_id)} className="text-tg-green hover:bg-tg-green/10 p-2 rounded-lg" title="تأیید"><UserCheck size={18} /></button>
                        <button onClick={() => rejectRequest(req.id)} className="text-tg-red hover:bg-tg-red/10 p-2 rounded-lg" title="رد"><XCircle size={18} /></button>
                      </div>
                    )
                  })}
                </div>
