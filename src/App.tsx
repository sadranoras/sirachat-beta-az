...
export default function App() {
  ...
  const openChatWithUser = useCallback(async (userId: string) => {
    ...
    if (userId === user.id) {
      ...
      if (!savedId) {
        const { data: created } = await supabase.from('chats').insert({ type: 'saved', title: 'پیام‌های ذخیره شده', created_by: user.id }).select('id').single()
        if (created?.id) {
          savedId = created.id
          await supabase.from('chat_members').insert({ chat_id: created.id, user_id: user.id, role: 'owner' })
        }
      }
      if (savedId) { setSelectedChatId(savedId); window.dispatchEvent(new Event('chat-list-reload')) }
      return
    }
    ...
    <div className="h-screen flex overflow-hidden bg-tg-bg">
      ...
      {showSettings && <SettingsPanel onClose={() => setShowSettings(false)} onOpenSavedMessages={async () => {
        ...
        if (savedId) {
          ...
          window.dispatchEvent(new Event('chat-list-reload'))
        }
      }} />}
