...
function TwoFactorGate({ onVerified }: { onVerified: () => void }) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (!factor) throw new Error('هیچ عامل تأیید دو مرحله‌ای تأییدشده‌ای پیدا نشد')
        ...
        if (!cancelled) setError(e?.message || 'خطا در آماده‌سازی تأیید دو مرحله‌ای')
      } finally {
  ...
  const verify = async () => {
    ...
    if (verifyError) {
      setError(verifyError.message || 'کد نادرست است')
      setLoading(false)
      return
    }
    onVerified()
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-tg-bg text-tg-text">در حال بررسی امنیت حساب...</div>
  if (error) return <div className="min-h-screen flex items-center justify-center bg-tg-bg p-4"><div className="bg-tg-panel rounded-2xl p-6 max-w-md w-full text-center"><p className="text-tg-red text-sm">{error}</p></div></div>
  return <div className="min-h-screen flex items-center justify-center bg-tg-bg p-4"><div className="bg-tg-panel rounded-2xl p-6 max-w-md w-full"><h2 className="text-lg font-bold text-tg-text mb-3 text-center">تأیید دو مرحله‌ای</h2><p className="text-tg-subtext text-sm mb-4 text-center">کد ۶ رقمی Authenticator را وارد کنید.</p><input autoFocus inputMode="numeric" maxLength={6} value={code} onChange={e => setCode(e.target.value.replace(/\\D/g,''))} className="w-full bg-tg-hover rounded-xl px-4 py-3 text-tg-text outline-none focus:ring-2 ring-tg-accent text-center tracking-widest" dir="ltr" /><button onClick={verify} disabled={loading || code.length!==6} className="w-full mt-3 bg-tg-accent text-white rounded-xl py-3 disabled:opacity-50">تأیید و ورود</button></div></div>
}

export default function App() {
  const { user, profile, loading } = useAuth()
  const [mfaVerified, setMfaVerified] = useState(false)
  useEffect(() => { setMfaVerified(!user ? false : false) }, [user])
  ...
  const openChatWithUser = useCallback(async (userId: string) => {
    if (!user) return
    if (userId === user.id) {
      const { data: savedChat } = await supabase.from('chats').select('id').eq('type', 'saved').eq('created_by', user.id).maybeSingle()
      if (savedChat?.id) setSelectedChatId(savedChat.id)
      return
    }
  ...
  const openChatWithUsername = useCallback(async (username: string) => {
    ...
    const { data: profileRow } = await supabase.from('profiles').select('id').eq('username', uname).maybeSingle()
    if (profileRow) { openChatWithUser((profileRow as any).id); return }
    const { data: chatRow } = await supabase.from('chats').select('id, is_private').eq('username', uname).maybeSingle()
    if (chatRow) {
      const chatId = (chatRow as any).id
      const isPrivate = (chatRow as any).is_private
      const { data: membership } = await supabase.from('chat_members').select('chat_id').eq('chat_id', chatId).eq('user_id', user?.id || '').maybeSingle()
      setPreviewChatId(''); setPreviewChat(null)
      if (membership) { setSelectedChatId(chatId) }
      else if (isPrivate) { setSelectedChatId(''); setPreviewChat({ id: chatId, isPrivate: true }) }
      else { setSelectedChatId(''); setPreviewChatId(chatId) }
      return
    }
  ...
  if (!mfaVerified) return <TwoFactorGate onVerified={() => setMfaVerified(true)} />
  ...
  return (
    <div className="h-screen flex overflow-hidden bg-tg-bg">
      ...
      {showSettings && <SettingsPanel onClose={() => setShowSettings(false)} onOpenSavedMessages={async () => {
        if (!user) return
        const { data } = await supabase.from('chats').select('id').eq('type', 'saved').eq('created_by', user.id).maybeSingle()
        if (data?.id) { setShowSettings(false); setSelectedChatId(data.id) }
      }} />}
      {showAdmin && <Suspense fallback={null}><AdminPanel onClose={() => setShowAdmin(false)} /></Suspense>}
      {profileUserId && <ProfileModal userId={profileUserId} onClose={() => setProfileUserId(null)} onMessage={() => { setProfileUserId(null); openChatWithUser(profileUserId) }} />}
